...
class NavMotionSafetyGate(Node):
    ...
    def rear_left_turn_allowed(self):
        ...
        return self.rear_reverse_allowed()

    def rear_right_turn_allowed(self):
        return self.rear_reverse_allowed()

    def ultrasonic_blocking_sides(self):
        ...
        for index, name in enumerate(("LEFT", "RIGHT")):
            if (
                    ...
                    > self.ultrasonic_timeout
                    or self.ultrasonic_latches[index].blocked):
                blocked.append(name)
        ...
        if guarded_sensors:
            ...
            if len(failed_indices) >= 2:
                return (
                    ...
                    "classifications=["
                    f"{self.classify_ultrasonic(0)},"
                    f"{self.classify_ultrasonic(1)}]"
                )
    ...
    def publish_ultrasonic_status(self):
        ...
        blocking_sides = self.ultrasonic_blocking_sides()
        state = "CLEAR" if reverse_allowed else "BLOCKED"
