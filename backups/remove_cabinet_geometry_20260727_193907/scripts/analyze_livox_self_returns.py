...
def main():
    ...
    parser.add_argument("--base-roll", type=float, default=15.732)
    parser.add_argument("--base-pitch", type=float, default=-0.611)
    # M12 (-X) faces vehicle right (-Y), so LiDAR +X faces vehicle left.
    parser.add_argument("--base-yaw", type=float, default=90.0)
    parser.add_argument("--self-margin", type=float, default=0.04)
    ...
    point_ranges = []
    near_points = 0
    progress_step = max(1, len(sampled) // 6)
    for sample_index, (message_id,) in enumerate(sampled, start=1):
        ...
        if (
            sample_index == 1
            or sample_index % progress_step == 0
            or sample_index == len(sampled)
        ):
            print(
                f"self_return_scan_progress: "
                f"{sample_index}/{len(sampled)} frames "
                f"near_points={near_points}",
                flush=True,
            )
    connection.close()
    ...
    roll = math.radians(args.base_roll)
    pitch = math.radians(args.base_pitch)
    yaw = math.radians(args.base_yaw)
    ...
    def sensor_to_base(point):
        translation = (args.base_x, args.base_y, args.base_z)
        return tuple(
            translation[row] + sum(
                rotation[row][column] * point[column]
    ...
    print(
        f"lidar_mount_base: xyz=({args.base_x:+.3f},{args.base_y:+.3f},"
        f"{args.base_z:+.3f})m rpy=({args.base_roll:+.3f},"
        f"{args.base_pitch:+.3f},{args.base_yaw:+.3f})deg"
    )
    for index, component in enumerate(components[:20], start=1):
        ...
        nearest = min(ranges)
        farthest = max(ranges)
        base_points = [sensor_to_base(point) for point in centers]
    ...
    vehicle_boxes = (
        ("chassis", (-0.38, 0.38), (-0.27, 0.27), (-0.05, 0.40)),
        ("cabinet", (0.39, 0.94), (-0.20, 0.20), (-0.03, 0.75)),
    )
    persistent_centers = {
        voxel: tuple(
            (coordinate + 0.5) * args.voxel for coordinate in voxel)
        for voxel in persistent
    }
    vehicle_voxels = {}
    vehicle_counts = collections.Counter()
    for voxel, center in persistent_centers.items():
        base_point = sensor_to_base(center)
        for label, x_limits, y_limits, z_limits in vehicle_boxes:
            if (
                x_limits[0] <= base_point[0] <= x_limits[1]
                and y_limits[0] <= base_point[1] <= y_limits[1]
                and z_limits[0] <= base_point[2] <= z_limits[1]
            ):
                vehicle_voxels[voxel] = center
                vehicle_counts[label] += 1
                break
    print(
        "vehicle_geometry: "
        "chassis=x[-0.38,+0.38] y[-0.27,+0.27] z[-0.05,+0.40]m; "
        "cabinet=x[+0.39,+0.94] y[-0.20,+0.20] z[-0.03,+0.75]m"
    )
    if vehicle_voxels:
        centers = list(vehicle_voxels.values())
        half_voxel = 0.5 * args.voxel
        sensor_min = [
            min(point[axis] for point in centers)
            - half_voxel - args.self_margin
            for axis in range(3)
        ]
        sensor_max = [
            max(point[axis] for point in centers)
            + half_voxel + args.self_margin
            for axis in range(3)
        ]
        print(
            f"vehicle_self_returns: voxels={len(vehicle_voxels)} "
            f"chassis={vehicle_counts['chassis']} "
            f"cabinet={vehicle_counts['cabinet']} "
            f"persistence_max="
            f"{max(persistent[voxel] for voxel in vehicle_voxels)}"
            f"/{len(sampled)}frames"
        )
        print(
            f"recommended_sensor_exclusion_with_"
            f"{args.self_margin:.2f}m_margin: "
            f"x=[{sensor_min[0]:+.3f},{sensor_max[0]:+.3f}] "
            f"y=[{sensor_min[1]:+.3f},{sensor_max[1]:+.3f}] "
            f"z=[{sensor_min[2]:+.3f},{sensor_max[2]:+.3f}]"
        )
        ...
        print(
            "vehicle_self_returns: none in persistent voxels; "
            "keep exclusion_box_enabled=false"
        )
    ...
    if args.check_range_gate:
        ...
        if outside:
            ...
            print(
                f"RESULT: FAIL vehicle self-returns have "
                f"{len(outside)}/{len(centers)} voxels beyond the "
        ...
        if centers:
            print(
                "range_gate_check: PASS all vehicle self-return voxels are "
                f"inside blind={args.mapping_blind:.3f}m"
            )
        else:
            print(
                "range_gate_check: PASS no persistent returns intersect "
                "the configured vehicle geometry"
            )
