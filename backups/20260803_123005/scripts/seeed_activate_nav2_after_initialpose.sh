...
validate_chassis_lidar_geometry() {
  ...
  echo "Rear sonar compact safety: sensor_to_tail=${ULTRASONIC_SENSOR_TO_REAR_EDGE_M}m hard_stop<=${ULTRASONIC_STOP_DISTANCE_M}m full_clear>=${ULTRASONIC_CLEAR_DISTANCE_M}m minimum_tail_clearance=${ULTRASONIC_TAIL_CLEARANCE_M}m"
  echo "Rear sonar maneuver guards: left turn=sensor_1(left), right turn=sensor_2(right), reverse=both; speed taper active between stop and clear."
}
