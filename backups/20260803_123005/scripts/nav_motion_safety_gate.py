...
class NavMotionSafetyGate(Node):
    ...
    def rear_reverse_allowed(self):
        return self.ultrasonic_sensor_allowed(0) and \
            self.ultrasonic_sensor_allowed(1)

    def ultrasonic_sensor_allowed(self, index):
        ...
        return (
            ...
            and not self.ultrasonic_latches[index].blocked
        )

    def rear_left_turn_allowed(self):
        # Positive base_link yaw sweeps the left rear corner backwards.
        return self.ultrasonic_sensor_allowed(0)

    def rear_right_turn_allowed(self):
        # Negative base_link yaw sweeps the right rear corner backwards.
        return self.ultrasonic_sensor_allowed(1)
    ...
    def guarded_ultrasonic_indices(
        ...
        if turn_guard_enabled:
            # base_link uses +Y left and +yaw counter-clockwise. Positive yaw
            # sweeps the left rear corner backwards; negative yaw sweeps the
            # right rear corner backwards.
            if angular_z > 0.001:
                indices.add(0)
            elif angular_z < -0.001:
                indices.add(1)
        ...
        if guarded_sensors:
            ...
            for index in guarded_sensors:
                ...
                if latch.blocked:
                    ...
                    return (
                        f"rear ultrasonic {condition} sensor_{index + 1} "
                        f"raw={value:.3f}m "
    ...
    def publish_ultrasonic_status(self):
        ...
        state = "CLEAR" if reverse_allowed else "PARTIAL_OR_BLOCKED"
        ...
        self.ultrasonic_status_pub.publish(String(data=(
            ...
            "turn_guard=[left:sensor_1,right:sensor_2]"
        )))
