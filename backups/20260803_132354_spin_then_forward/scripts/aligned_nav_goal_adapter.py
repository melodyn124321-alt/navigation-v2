    ...
    async def run_segmented_spin(self, outer_handle, angle, phase):
        ...
        for spin_index in range(spin_steps):
            ...
            if spin_result.status != GoalStatus.STATUS_SUCCEEDED:
                self.publish_status(
                    "SPIN_BLOCKED bounded small-spin failed "
                    f"phase={phase} step={spin_index + 1}/{spin_steps} "
        ...
        try:
            ...
            if base_pose is not None and self.corridor_is_clear(
                ...
                if direct_route_mode == "forward":
                    # Ordinary forward planning is position-first 2D A*. It
                    # avoids forcing a short clear goal through an infeasible
                    # Dubin terminal-yaw constraint. The controller is still
                    # forward-only and final yaw is handled separately by
                    # bounded measured spin segments.
            ...
            if direct_route:
                ...
                self.publish_status(
                    "FORWARD_ROUTE direct corridor is clear; using a "
                    "collision-checked forward 2D A* path to the target "
                    "position without pre-spin; final yaw is separate")
            else:
                self.publish_status(
