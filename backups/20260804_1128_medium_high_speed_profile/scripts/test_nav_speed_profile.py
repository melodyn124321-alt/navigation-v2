...
def main():
    ...
    assert start.state == "START" and math.isclose(start.cap, 0.05)

    cruise = decide(0.31, 2.0)
    assert cruise.state == "CRUISE" and math.isclose(cruise.cap, 0.16)

    approach = decide(1.0, 0.375)
    assert approach.state == "APPROACH"
    assert math.isclose(approach.cap, 0.0975)

    final = decide(1.0, 0.0)
    assert final.state == "APPROACH" and math.isclose(final.cap, 0.035)

    short_goal = decide(0.0, 0.20)
    assert short_goal.state == "START_APPROACH"
    assert math.isclose(short_goal.cap, 0.05)

    print(
        "NAV_SPEED_PROFILE_UNIT_PASS "
        "start=0.050m/s@0.30m cruise=0.160m/s "
        "approach=0.75m->0.035m/s short_goal=LOW_SPEED"
    )
