...
class NavMotionSafetyGate(Node):
    def __init__(self):
        ...
        self.ultrasonic_stop_distance = float(self.declare_parameter(
            "ultrasonic_stop_distance", 0.60).value)
        self.ultrasonic_clear_distance = float(self.declare_parameter(
            "ultrasonic_clear_distance", 0.75).value)
    ...
    def rear_reverse_allowed(self):
        ...
        for index, stamp in enumerate(self.last_ultrasonic_times):
            if self.elapsed(stamp) > self.ultrasonic_timeout:
                return False
            if self.ultrasonic_latches[index].blocked:
                return False
        return True
    ...
    def publish_ultrasonic_status(self):
        ...
        safe = self.rear_reverse_allowed()
        state = "CLEAR" if safe else "BLOCKED"
        self.ultrasonic_reverse_allowed_pub.publish(Bool(data=safe))
        self.ultrasonic_status_pub.publish(String(data=(
            f"state={state} healthy={str(self.ultrasonic_healthy).lower()} "
            f"reverse_allowed={str(safe).lower()} "
