...
def main():
    ...
    update(latch, 0.40, 5.0)
    assert latch.blocked
    update(latch, 0.80, 6.6)
    update(latch, 0.90, 6.85)
    assert latch.blocked
    update(latch, 1.00, 7.10)
    assert not latch.blocked

    update(latch, math.nan, 8.0)
    assert latch.blocked
    update(latch, 0.65, 10.0)
    assert latch.blocked
    ...
    print(
        ...
        "stop=0.60m clear=0.75m hold=1.50s "
