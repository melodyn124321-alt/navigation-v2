...
grep -q '1.57' <<<"${adapter_alignment_max}" || failed=true
grep -q '3.14' <<<"${adapter_final_alignment_max}" || failed=true
...
# The deployed planner is deliberately Navfn A*. Final yaw and checked
# straight reverse are handled by aligned_nav_goal_adapter, so the old
# Smac/DUBIN-only parameters are not applicable.
...
grep -q '0.6' <<<"${sonar_stop_distance}" || failed=true
grep -q '0.75' <<<"${sonar_clear_distance}" || failed=true
...
grep -q '0.3' <<<"${sonar_tail_clearance}" || failed=true
