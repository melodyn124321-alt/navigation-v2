...
LIDAR_ROLL_RAD="${LIDAR_ROLL_RAD:-0.274581016}"
LIDAR_PITCH_RAD="${LIDAR_PITCH_RAD:--0.010669779}"
# M12 (-X) faces vehicle right (-Y): LiDAR +X faces vehicle left (+Y).
LIDAR_YAW_RAD="${LIDAR_YAW_RAD:-1.570796327}"
# Defaults place both sonars on the rear face of the 0.72 x 0.50 m chassis.
# Override these values with measured mount coordinates before final operation.
ULTRASONIC_X_M="${ULTRASONIC_X_M:--0.38}"
ULTRASONIC_LEFT_Y_M="${ULTRASONIC_LEFT_Y_M:-0.18}"
ULTRASONIC_RIGHT_Y_M="${ULTRASONIC_RIGHT_Y_M:--0.18}"
ULTRASONIC_Z_M="${ULTRASONIC_Z_M:-0.20}"
...
stop_old_navigation() {
  ...
  remaining="$(ps -eo pid=,args= | awk '
    /planner_server|controller_server|smoother_server|behavior_server|bt_navigator|waypoint_follower|velocity_smoother|collision_monitor|lifecycle_manager_navigation|nav_obstacle_cloud_republisher.py|aligned_nav_goal_adapter.py|nav_goal_marker_from_plan.py|nav_obstacle_block_alarm.py|ndt_goal_controller.py|nav_motion_safety_gate.py|static_transform_publisher.*nav_lidar|static_transform_publisher.*rear_ultrasonic/ &&
    $0 !~ /awk/ &&
...
start_motion_gate() {
  ...
  for attempt in 1 2; do
    start_node nav_motion_safety_gate python3 \
      ...
      -p ultrasonic_stop_distance:=0.35 \
      -p ultrasonic_clear_distance:=0.45
...
wait_for_topic_connection /ultrasonic/sensor_1/range 1 0
wait_for_topic_connection /ultrasonic/sensor_2/range 1 0
wait_for_topic_connection /ultrasonic/min_range 1 0
wait_for_topic_connection /ultrasonic/healthy 1 0
...
start_node nav_obstacle_block_alarm python3 "${OBSTACLE_ALARM}" --ros-args \
  -p stop_radius:=0.60 -p min_height:=0.05 -p max_height:=1.60 \
