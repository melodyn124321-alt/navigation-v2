...
approach_footprint="$(get_param_with_retry /collision_monitor FootprintApproach.footprint_topic || true)"
...
printf 'footprint approach topic: %s\n' "${approach_footprint:-missing}"
...
grep -q 'RotationStopZone' <<<"${collision_polygons}" || failed=true
...
grep -q '/local_costmap/published_footprint' <<<"${approach_footprint}" || failed=true
...
grep -q '0.05' <<<"${profile_start_speed}" || failed=true
grep -q '0.16' <<<"${profile_cruise_speed}" || failed=true
grep -q '0.75' <<<"${profile_approach_distance}" || failed=true
grep -q '0.035' <<<"${profile_approach_min}" || failed=true
