...
BASE_TO_LIDAR_ROLL_RAD="${BASE_TO_LIDAR_ROLL_RAD:--0.03197443}"
BASE_TO_LIDAR_PITCH_RAD="${BASE_TO_LIDAR_PITCH_RAD:-0.43653303}"
BASE_TO_LIDAR_YAW_RAD="${BASE_TO_LIDAR_YAW_RAD:-0.0}"
LOCALIZATION_TO_BASE_ROLL_RAD="${LOCALIZATION_TO_BASE_ROLL_RAD:-0.03528057}"
LOCALIZATION_TO_BASE_PITCH_RAD="${LOCALIZATION_TO_BASE_PITCH_RAD:--0.43629457}"
LOCALIZATION_TO_BASE_YAW_RAD="${LOCALIZATION_TO_BASE_YAW_RAD:--0.01491409}"
LOCALIZATION_TO_BASE_X_M="${LOCALIZATION_TO_BASE_X_M:--0.5105783}"
LOCALIZATION_TO_BASE_Y_M="${LOCALIZATION_TO_BASE_Y_M:-0.0822838}"
LOCALIZATION_TO_BASE_Z_M="${LOCALIZATION_TO_BASE_Z_M:--1.0085332}"
TF_BRIDGE_INPUT_TIMEOUT="${TF_BRIDGE_INPUT_TIMEOUT:-10.0}"
VERIFY_LIDAR_MOUNT="${VERIFY_LIDAR_MOUNT:-${VERIFY_LIDAR_LEVEL:-true}}"
EXPECTED_LIDAR_ROLL_DEG="${EXPECTED_LIDAR_ROLL_DEG:--1.832}"
EXPECTED_LIDAR_PITCH_DEG="${EXPECTED_LIDAR_PITCH_DEG:-25.012}"
...
"${KS109_CONTROL}" start
if ! timeout 8 "${ROS_ENV}" ros2 topic echo --once /ultrasonic/healthy \
...
for topic in /ultrasonic/sensor_1/range /ultrasonic/sensor_2/range; do
  if ! timeout 8 "${ROS_ENV}" ros2 topic echo --once "${topic}" \
    >/dev/null 2>&1; then
...
if [[ "${VERIFY_LIDAR_MOUNT}" == "true" ]]; then
  echo "Checking the stationary live LiDAR against the calibrated +25.012 deg downward mounting pitch..."
