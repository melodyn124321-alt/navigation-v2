...
case "${ACTION}" in
  start)
    ...
    "${ROS_ENV}" python3 "${ROS_ROOT}/scripts/verify_live_livox_mount.py" \
  ...
  stop)
    ...
    if [[ -r "${UDP_BASELINE_FILE}" ]]; then
      ...
      echo "UDP receive errors during recording: in_errors=${udp_in_delta} rcvbuf_errors=${udp_buf_delta}"
    ...
    "${ROS_ENV}" python3 "${ROS_ROOT}/scripts/analyze_livox_bag_mount.py" \
      "${BAG_PATH}" \
      --expected-roll "${EXPECTED_LIDAR_ROLL_DEG}" \
      --expected-pitch "${EXPECTED_LIDAR_PITCH_DEG}" \
      --max-angle-error "${LIDAR_MOUNT_TOLERANCE_DEG}"
  ...
  status)
    ...
    ;;
  *)
    echo "Usage: $0 {start|stop|status} BAG_NAME" >&2
    exit 1
    ;;
esac
