...
EXPECTED_LIDAR_YAW_DEG="${EXPECTED_LIDAR_YAW_DEG:-0.0}"
...
if [[ "${activation_pid}" =~ ^[0-9]+$ ]] \
  ...
  echo "REFUSED: Nav2 activation is still running (pid=${activation_pid})." >&2
    echo "Run seeed_nav2_activation_job.sh wait 300 before verification." >&2
  exit 2
fi
...
grep -q '0.94' <<<"${local_footprint}" || failed=true
grep -q '0.27' <<<"${local_footprint}" || failed=true
grep -q '0.94' <<<"${global_footprint}" || failed=true
grep -q '0.27' <<<"${global_footprint}" || failed=true
grep -q '1.08' <<<"${rotation_zone}" || failed=true
