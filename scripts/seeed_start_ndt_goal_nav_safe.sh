...
NDT_RUNTIME_BLOCK_FITNESS="${NDT_RUNTIME_BLOCK_FITNESS:-0.15}"
NDT_RUNTIME_CLEAR_FITNESS="${NDT_RUNTIME_CLEAR_FITNESS:-0.12}"
NDT_RUNTIME_BAD_HOLD_SEC="${NDT_RUNTIME_BAD_HOLD_SEC:-1.0}"
NDT_RUNTIME_CLEAR_SAMPLES="${NDT_RUNTIME_CLEAR_SAMPLES:-5}"
...
start_navigation_nodes() {
  ...
  setsid -f "${ROS_ENV}" env FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ...
    -p localization_to_base_x_m:=-0.115311164 \
    -p localization_to_base_y_m:=0.468093861 \
