...
class FastlioVizCloudThrottle(Node):
    def __init__(self):
        ...
        self.publish_once = bool(self.declare_parameter(
            "publish_once", False).value)
        input_reliability = str(self.declare_parameter(
            "input_reliability", "reliable").value).strip().lower()
        ...
        self.get_logger().info(
            ...
            f"input QoS={input_reliability}/volatile depth=1, "
    ...
    def on_cloud(self, message):
        ...
        if self.publish_once and self.published > 0:
            return
        now = time.monotonic()
