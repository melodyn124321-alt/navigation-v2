...
kill_ros_wrappers() {
  ...
  pids="$(ps -eo pid=,args= | awk \
    -v pat="$pattern" -v self_pid="$$" -v parent_pid="${PPID}" \
    '$1 != self_pid && $1 != parent_pid &&
     $0 ~ pat && $0 !~ /awk -v pat/ {print $1}')"
  if [ -n "${pids}" ]; then
...
force_kill_matches() {
  ps -eo pid=,comm=,args= | awk '
    /livox_ros_driver2_node|fastlio_mapping|fastlio_live_watchdog.py|fastlio_chassis_odometry.py|fastlio_viz_cloud_throttle.py|pcd_ndt_localizer|nav2_bringup|map_server|planner_server|controller_server|smoother_server|behavior_server|bt_navigator|waypoint_follower|velocity_smoother|velocity_smooth|collision_monitor|lifecycle_manag|lifecycle_manager|component_container|ranger_base_node|map_camera_init_bridge.py|map_odom_tf_bridge.py|map_odom_tf_bridge|nav_tf_direct_relay.py|odom_tf_republisher.py|nav_static_tf_bootstrap.py|nav_identity_tf_publisher.py|relocalization_tf_republisher.py|safe_cmd_vel_bridge.py|nav_goal_marker_from_plan.py|nav_obstacle_block_alarm.py|aligned_nav_goal_adapter.py|rviz_goal_pose_bridge.py|ndt_goal_controller.py|nav_motion_safety_gate.py|nav_obstacle_cloud_republisher.py|hn_local_costmap_republisher.py|cloud_callback_probe|cloud_geometry_inspector|static_transform_publisher.*nav_lidar|ros2 bag play|ros2 bag record|rviz2|apport/ &&
    $0 !~ /awk/ &&
...
kill_ros_wrappers "fastlio_live_watchdog.py"
kill_ros_wrappers "fastlio_chassis_odometry.py"
kill_ros_wrappers "fastlio_viz_cloud_throttle.py"
...
kill_ros_wrappers "rviz_goal_pose_bridge.py"
kill_ros_wrappers "seeed_refresh_nav_goal_adapter.sh"
kill_ros_wrappers "nav_motion_safety_gate.py"
kill_ros_wrappers "nav_obstacle_cloud_republisher.py"
kill_ros_wrappers "hn_local_costmap_republisher.py"
