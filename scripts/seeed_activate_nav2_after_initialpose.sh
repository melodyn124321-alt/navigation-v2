...
# Activation checks must use direct DDS participants. The ros2cli daemon can
# retain an invalid rclpy context after the navigation stack is restarted.
...
TERMINAL_DRIVE_VERIFIER="${TERMINAL_DRIVE_VERIFIER:-${ROS_ROOT}/scripts/verify_terminal_drive_disarmed.py}"
OBSTACLE_REPUBLISHER="${OBSTACLE_REPUBLISHER:-${ROS_ROOT}/scripts/nav_obstacle_cloud_republisher.py}"
HN_LOCAL_COSTMAP_REPUBLISHER="${HN_LOCAL_COSTMAP_REPUBLISHER:-${ROS_ROOT}/scripts/hn_local_costmap_republisher.py}"
...
# The far end of manual_003 has less overlapping 3D structure than the start
# area.  There the measured pose remains centimetre-stable while the absolute
# NDT score sits around 0.12-0.20.  Keep a margin below the localizer's 0.25
# rejection gate, require a sustained failure, and pair this with the pose-
# stability check below instead of treating every regional score shift as a
# lost localization.
MAX_NDT_FITNESS="${MAX_NDT_FITNESS:-0.18}"
NDT_RUNTIME_BLOCK_FITNESS="${NDT_RUNTIME_BLOCK_FITNESS:-0.22}"
NDT_RUNTIME_CLEAR_FITNESS="${NDT_RUNTIME_CLEAR_FITNESS:-0.18}"
NDT_RUNTIME_BAD_HOLD_SEC="${NDT_RUNTIME_BAD_HOLD_SEC:-2.0}"
NDT_RUNTIME_CLEAR_SAMPLES="${NDT_RUNTIME_CLEAR_SAMPLES:-3}"
NDT_TERMINAL_DISTANCE_M="${NDT_TERMINAL_DISTANCE_M:-0.20}"
NDT_TERMINAL_BLOCK_FITNESS="${NDT_TERMINAL_BLOCK_FITNESS:-0.35}"
NDT_TERMINAL_CLEAR_FITNESS="${NDT_TERMINAL_CLEAR_FITNESS:-0.32}"
...
BT_DISCOVERY_SETTLE_SEC="${BT_DISCOVERY_SETTLE_SEC:-8.0}"
NODE_RETRY_SETTLE_SEC="${NODE_RETRY_SETTLE_SEC:-3.0}"
CORE_ACTION_TIMEOUT_SEC="${CORE_ACTION_TIMEOUT_SEC:-30.0}"
...
# Re-measured 2026-08-06: each rear sonar is 0.15 m forward of the rear edge,
...
ULTRASONIC_SENSOR_TO_REAR_EDGE_M="${ULTRASONIC_SENSOR_TO_REAR_EDGE_M:-0.15}"
ULTRASONIC_ABOVE_CHASSIS_M="${ULTRASONIC_ABOVE_CHASSIS_M:-0.20}"
ULTRASONIC_X_M="${ULTRASONIC_X_M:--0.210}"
...
# The re-mounted sonars show a repeatable 0.08-0.12 m body echo; this band is
# ignored only as self-reflection. Other values <=0.22 m remain hard stops.
ULTRASONIC_SELF_ECHO_MIN_M="${ULTRASONIC_SELF_ECHO_MIN_M:-0.08}"
ULTRASONIC_SELF_ECHO_MAX_M="${ULTRASONIC_SELF_ECHO_MAX_M:-0.12}"
...
for path in "${NAV2_PARAMS}" "${NAV_YAML}" "${TF_CHECKER}" \
  ...
  "${NAV2_LIFECYCLE}" "${NAV2_ACTION_CHECKER}" "${COSTMAP_INSPECTOR}" \
  "${TERMINAL_DRIVE_VERIFIER}" \
  "${OBSTACLE_REPUBLISHER}" "${HN_LOCAL_COSTMAP_REPUBLISHER}" \
  "${ALIGNED_GOAL_ADAPTER}" \
...
wait_for_typed_message() {
  ...
  for attempt in $(seq 1 6); do
    timeout 8 "${ROS_ENV}" env \
      ...
      ros2 topic echo --no-daemon "${topic}" "${type}" --once \
      >"${output}" 2>&1 || true
    if [[ -s "${output}" ]] \
      && { [[ -z "${required_pattern}" ]] \
...
stop_old_navigation() {
  ...
  remaining="$(ps -eo pid=,args= | awk '
    /planner_server|controller_server|smoother_server|behavior_server|bt_navigator|waypoint_follower|velocity_smoother|collision_monitor|lifecycle_manager_navigation|nav_obstacle_cloud_republisher.py|hn_local_costmap_republisher.py|nav_tf_direct_relay.py|aligned_nav_goal_adapter.py|rviz_goal_pose_bridge.py|nav_goal_marker_from_plan.py|nav_obstacle_block_alarm.py|ndt_goal_controller.py|nav_motion_safety_gate.py|static_transform_publisher.*nav_lidar|static_transform_publisher.*rear_ultrasonic/ &&
    $0 !~ /awk/ &&
...
start_motion_gate() {
  ...
  for attempt in 1 2; do
    start_node nav_motion_safety_gate python3 \
      ...
      -p max_fitness:="${MAX_NDT_FITNESS}" \
      -p fitness_block_threshold:="${NDT_RUNTIME_BLOCK_FITNESS}" \
      -p fitness_clear_threshold:="${NDT_RUNTIME_CLEAR_FITNESS}" \
      -p fitness_bad_hold_sec:="${NDT_RUNTIME_BAD_HOLD_SEC}" \
      -p fitness_clear_samples_required:="${NDT_RUNTIME_CLEAR_SAMPLES}" \
      -p terminal_fitness_distance:="${NDT_TERMINAL_DISTANCE_M}" \
      -p terminal_fitness_block_threshold:="${NDT_TERMINAL_BLOCK_FITNESS}" \
      -p terminal_fitness_clear_threshold:="${NDT_TERMINAL_CLEAR_FITNESS}" \
...
stop_motion_gate() {
  ...
  echo "Motion gate did not stop cleanly: ${remaining}" >&2
  return 1
}

probe_motion_gate_readiness_disarmed() {
  local arm_output disarm_output
  arm_output="$(timeout 18 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ROS2CLI_DISABLE_DAEMON=1 \
    ros2 service call /set_nav_motion_enabled \
      std_srvs/srv/SetBool '{data: true}' 2>&1 || true)"
  disarm_output="$(timeout 18 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ROS2CLI_DISABLE_DAEMON=1 \
    ros2 service call /set_nav_motion_enabled \
      std_srvs/srv/SetBool '{data: false}' 2>&1 || true)"
  printf '%s\n' "${arm_output}"
  printf '%s\n' "${disarm_output}"
  grep -Eq 'success=(True|true)' <<<"${arm_output}" \
    && grep -Eq 'success=(True|true)' <<<"${disarm_output}"
}

echo "[1/11] Checking live NDT localization..."
# NDT can report LOCALIZED without the physical chassis driver or the TF
# bridge.  Fail fast and explain that startup is incomplete instead of waiting
# for the combined stability verifier and incorrectly blaming 2D Pose.
if ! timeout 12 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ROS2CLI_DISABLE_DAEMON=1 \
    ros2 topic echo --no-daemon /odom nav_msgs/msg/Odometry --once \
    >/tmp/nav2_ranger_odom_check.txt 2>&1; then
  cat /tmp/nav2_ranger_odom_check.txt >&2 || true
  echo "No live Ranger /odom; the base stack stopped before the chassis/TF stage." >&2
  echo "Do not reset 2D Pose. Restore live CAN traffic, then rerun the complete" >&2
  echo "seeed_start_manual_002_nav_safe.sh command with START_RANGER=true." >&2
  exit 1
fi
if ! pgrep -f '/nav_tf_bridge/map_odom_tf_bridge|ros2 run nav_tf_bridge map_odom_tf_bridge' \
    >/dev/null; then
  echo "map_odom_tf_bridge is not running; map->odom cannot exist." >&2
  echo "Do not reset 2D Pose. Rerun the complete base-stack start command." >&2
  exit 1
fi
if ! timeout 30 "${ROS_ENV}" env \
  FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
  ROS2CLI_DISABLE_DAEMON=1 \
  ros2 topic echo --no-daemon \
    /relocalization_odom nav_msgs/msg/Odometry --once \
...
stability_output="$(timeout 25 "${ROS_ENV}" env \
  FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
  ...
  --max-fitness "${MAX_NDT_FITNESS}" 2>&1)" || stability_rc=$?
printf '%s\n' "${stability_output}"
if (( ${stability_rc:-0} != 0 )); then
  echo "Localization stability verification failed. Nav2 remains stopped." >&2
  if grep -Eq 'tf_successes=0([[:space:]]|$)' <<<"${stability_output}"; then
    echo "NDT is LOCALIZED, but map->base_link TF is completely absent." >&2
    echo "Check Ranger /odom and map_odom_tf_bridge; resetting 2D Pose will not fix it." >&2
  else
...
# map_server normally publishes once as it becomes active. On a cold boot the
# large transient-local map sample can precede the planner's static-layer
# subscription and fail to replay over Fast DDS. Restart it after the planner
# is active so the static layer receives a fresh live sample, then prove that
# the resulting global costmap contains the localized chassis pose.
echo "Refreshing the static map after the planner subscription is live..."
stop_managed_node map_server
start_node nav2_map_server ros2 run nav2_map_server map_server --ros-args \
  -p use_sim_time:=false \
  -p yaml_filename:="${NAV_YAML}" \
...
if ! timeout "${NODE_ACTIVATION_TIMEOUT}" "${ROS_ENV}" env \
  FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
  python3 "${NAV2_LIFECYCLE}" --timeout "${NODE_LIFECYCLE_TIMEOUT}" \
    --post-configure-settle 2 map_server \
  > "${ROS_ROOT}/logs/nav2_lifecycle_map_server_refresh.log" 2>&1; then
  cat "${ROS_ROOT}/logs/nav2_lifecycle_map_server_refresh.log" >&2 || true
  tail -n 120 "${ROS_ROOT}/logs/nav2_map_server.log" >&2 || true
  echo "map_server refresh failed; motion remains DISARMED." >&2
  exit 1
fi
if ! timeout 30 "${ROS_ENV}" env \
  FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
  python3 "${COSTMAP_INSPECTOR}" --require-start-passable --timeout 22 \
  > "${ROS_ROOT}/logs/nav2_global_costmap_start_check.log" 2>&1; then
  cat "${ROS_ROOT}/logs/nav2_global_costmap_start_check.log" >&2 || true
  echo "The refreshed global costmap is missing, incomplete, or does not contain the localized chassis pose." >&2
  echo "Motion remains DISARMED." >&2
  exit 1
fi
cat "${ROS_ROOT}/logs/nav2_global_costmap_start_check.log"

# The rolling local costmap is native to odom. Publishing it directly to a
# remote RViz can overflow its TF filter while map<-odom changes. Republish the
# complete grid with its origin transformed into map so HN retains a stable,
# live local-layer display without changing either Nav2 costmap internally.
stop_managed_node hn_local_costmap_republisher
start_node hn_local_costmap_republisher \
  /usr/bin/python3 "${HN_LOCAL_COSTMAP_REPUBLISHER}"
local_costmap_display_ready=false
for attempt in $(seq 1 12); do
  local_costmap_status="$(timeout 5 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ros2 topic echo /hn_local_costmap_status std_msgs/msg/String --once \
      2>/dev/null || true)"
  if grep -q 'data: READY ' <<<"${local_costmap_status}"; then
    local_costmap_display_ready=true
    printf '%s\n' "${local_costmap_status}"
    break
  fi
  echo "Waiting for the HN map-frame local costmap (${attempt}/12)..."
  sleep 1
done
if [[ "${local_costmap_display_ready}" != true ]]; then
  printf '%s\n' "${local_costmap_status:-no status}" >&2
  echo "HN local costmap display relay is unavailable; motion remains DISARMED." >&2
  exit 1
fi

# The gate is intentionally started early so no unguarded velocity path can
# exist. Fast DDS can still match its best-effort NDT endpoints without
# delivering samples on a busy cold boot. Restart it now that every producer is
# live, then exercise its prerequisite-aware arm service with no active goal.
# The probe immediately disarms again, so physical /cmd_vel remains zero.
motion_gate_data_ready=false
for attempt in 1 2 3; do
  stop_motion_gate || true
  if start_motion_gate; then
    # On a cold boot, discovery of the HN heartbeat can take longer than the
    # local topics. Keep this DISARMED gate alive long enough to receive it;
    # restarting every few seconds resets discovery and can create a permanent
    # stale-heartbeat loop. Every successful probe immediately disarms again.
    for warmup_attempt in $(seq 1 10); do
      if probe_motion_gate_readiness_disarmed \
        > "${ROS_ROOT}/logs/nav2_motion_gate_readiness_check.log" 2>&1; then
        motion_gate_data_ready=true
        break 2
      fi
      cat "${ROS_ROOT}/logs/nav2_motion_gate_readiness_check.log" >&2 || true
      echo "Motion gate inputs are still warming " \
        "(${warmup_attempt}/10 on gate ${attempt}/3); remaining DISARMED." >&2
      sleep 2
    done
  fi
  cat "${ROS_ROOT}/logs/nav2_motion_gate_readiness_check.log" >&2 || true
  echo "Motion gate data callbacks are incomplete on attempt ${attempt}/3; restarting while DISARMED." >&2
done
cat "${ROS_ROOT}/logs/nav2_motion_gate_readiness_check.log" || true
if [[ "${motion_gate_data_ready}" != true ]]; then
  echo "Motion gate cannot verify its own localization/chassis/sensor inputs; motion remains DISARMED." >&2
  exit 1
fi

# A lifecycle-active behavior_server is not sufficient: on some cold boots
# its private TF listener misses the Ranger odom publisher and every terminal
# DriveOnHeading action aborts with "odom frame does not exist". Exercise both
# behavior actions while the physical gate is still DISARMED. If discovery is
# stale, restart only behavior_server and retry; never bypass the verification.
terminal_behavior_ready=false
for attempt in 1 2 3; do
  if timeout 50 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    /usr/bin/python3 "${TERMINAL_DRIVE_VERIFIER}" \
    > "${ROS_ROOT}/logs/nav2_terminal_drive_disarmed_check.log" 2>&1; then
    terminal_behavior_ready=true
    break
  fi
  cat "${ROS_ROOT}/logs/nav2_terminal_drive_disarmed_check.log" >&2 || true
  echo "Behavior TF/action self-test failed on attempt ${attempt}/3; restarting /behavior_server while motion is DISARMED." >&2
  if ! start_and_activate_managed_node behavior_server; then
    echo "Failed to restart /behavior_server." >&2
    continue
  fi
  sleep 3
done
cat "${ROS_ROOT}/logs/nav2_terminal_drive_disarmed_check.log" || true
if [[ "${terminal_behavior_ready}" != true ]]; then
  echo "Behavior actions cannot consume odom TF; motion remains DISARMED." >&2
  exit 1
fi
...
for attempt in 1 2 3; do
  ...
  start_node aligned_nav_goal_adapter python3 "${ALIGNED_GOAL_ADAPTER}" --ros-args \
    ...
    -p terminal_drive_retry_count:=2 \
    -p terminal_drive_retry_delay:=1.5 \
...
start_node rviz_goal_pose_bridge python3 "${RVIZ_GOAL_BRIDGE}" --ros-args \
  -p goal_topic:=/goal_pose_relay \
  -p action_name:=/aligned_navigate_to_pose \
  -p required_frame:=map \
  -p goal_inbox_path:="${ROS_ROOT}/logs/hn_goal_pose_inbox.json" \
  -p duplicate_window:=10.0 \
  -p goal_inbox_max_age:=10.0 \
  -p queued_handoff_delay:=1.0 \
  -p queued_retry_limit:=5
wait_for_topic_connection /goal_pose_relay 0 1
goal_pose_endpoints="$(
  timeout 15 "${ROS_ENV}" env \
    FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ROS2CLI_DISABLE_DAEMON=1 \
    ros2 topic info /goal_pose_relay -v 2>/dev/null || true
)"
...
for topic in /nav_obstacle_cloud /local_costmap/costmap /hn_local_costmap \
