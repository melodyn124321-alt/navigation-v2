...
EXPECTED_SONAR_X_M="${EXPECTED_SONAR_X_M:--0.210}"
...
check_topic /nav_speed_profile_status 0
...
grep -q 'sensor_to_tail_m=0.150' <<<"${sonar_status}" || failed=true
...
grep -q 'self_echo_m=\[0.080,0.120\]' <<<"${sonar_status}" || failed=true
...
approach_action="$(get_param_with_retry /collision_monitor FootprintApproach.action_type || true)"
approach_points="$(get_param_with_retry /collision_monitor FootprintApproach.points || true)"
approach_ttc="$(get_param_with_retry /collision_monitor FootprintApproach.time_before_collision || true)"
...
printf 'footprint approach action: %s\n' "${approach_action:-missing}"
printf 'footprint approach points: %s\n' "${approach_points:-missing}"
printf 'footprint approach TTC: %s\n' "${approach_ttc:-missing}"
...
grep -q 'FootprintApproach' <<<"${collision_polygons}" || failed=true
if grep -q 'RotationStopZone' <<<"${collision_polygons}"; then
  echo "Direction-blind RotationStopZone must not be active." >&2
  failed=true
fi
if grep -q 'FrontStopZone' <<<"${collision_polygons}"; then
  echo "Fixed FrontStopZone must not be active during pure rotation." >&2
  failed=true
fi
grep -q 'approach' <<<"${approach_action}" || failed=true
grep -q -- '-0.38' <<<"${approach_points}" || failed=true
grep -q '0.27' <<<"${approach_points}" || failed=true
grep -q '2.5' <<<"${approach_ttc}" || failed=true
...
adapter_spin_effective_rate="$(get_param_with_retry /aligned_nav_goal_adapter small_spin_effective_min_angular_speed || true)"
adapter_spin_timeout_factor="$(get_param_with_retry /aligned_nav_goal_adapter small_spin_timeout_factor || true)"
adapter_spin_timeout_margin="$(get_param_with_retry /aligned_nav_goal_adapter small_spin_timeout_margin || true)"
adapter_use_map_spin="$(get_param_with_retry /aligned_nav_goal_adapter use_map_yaw_spin || true)"
adapter_map_spin_topic="$(get_param_with_retry /aligned_nav_goal_adapter map_spin_command_topic || true)"
adapter_map_spin_speed="$(get_param_with_retry /aligned_nav_goal_adapter map_spin_angular_speed || true)"
adapter_reverse_allowance="$(get_param_with_retry /aligned_nav_goal_adapter reverse_time_allowance_factor || true)"
adapter_use_terminal_handoff="$(get_param_with_retry /aligned_nav_goal_adapter use_terminal_handoff || true)"
...
adapter_final_position_tolerance="$(get_param_with_retry /aligned_nav_goal_adapter final_alignment_position_tolerance || true)"
adapter_final_confirmations="$(get_param_with_retry /aligned_nav_goal_adapter final_pose_confirmations || true)"
adapter_terminal_allowance="$(get_param_with_retry /aligned_nav_goal_adapter terminal_time_allowance_factor || true)"
adapter_collision_abort="$(get_param_with_retry /aligned_nav_goal_adapter collision_block_abort_timeout || true)"
...
pre_collision_topic="$(get_param_with_retry /nav_motion_safety_gate pre_collision_topic || true)"
...
profile_enabled="$(get_param_with_retry /nav_motion_safety_gate speed_profile_enabled || true)"
profile_start_distance="$(get_param_with_retry /nav_motion_safety_gate start_slow_distance || true)"
profile_start_speed="$(get_param_with_retry /nav_motion_safety_gate start_max_linear || true)"
profile_cruise_speed="$(get_param_with_retry /nav_motion_safety_gate max_linear || true)"
profile_approach_distance="$(get_param_with_retry /nav_motion_safety_gate approach_slow_distance || true)"
profile_approach_min="$(get_param_with_retry /nav_motion_safety_gate approach_min_linear || true)"
...
printf 'adapter spin effective rate: %s\n' "${adapter_spin_effective_rate:-missing}"
printf 'adapter spin timeout factor: %s\n' "${adapter_spin_timeout_factor:-missing}"
printf 'adapter spin timeout margin: %s\n' "${adapter_spin_timeout_margin:-missing}"
printf 'adapter use map-yaw spin: %s\n' "${adapter_use_map_spin:-missing}"
printf 'adapter map-spin topic: %s\n' "${adapter_map_spin_topic:-missing}"
printf 'adapter map-spin speed: %s\n' "${adapter_map_spin_speed:-missing}"
printf 'adapter reverse allowance factor: %s\n' "${adapter_reverse_allowance:-missing}"
printf 'adapter terminal handoff enabled: %s\n' "${adapter_use_terminal_handoff:-missing}"
...
printf 'adapter final position tolerance: %s\n' "${adapter_final_position_tolerance:-missing}"
printf 'adapter final confirmations: %s\n' "${adapter_final_confirmations:-missing}"
printf 'adapter terminal allowance factor: %s\n' "${adapter_terminal_allowance:-missing}"
printf 'adapter collision abort timeout: %s\n' "${adapter_collision_abort:-missing}"
printf 'BT XML: %s\n' "${bt_xml:-missing}"
...
printf 'pre-collision command topic: %s\n' "${pre_collision_topic:-missing}"
...
printf 'speed profile enabled: %s\n' "${profile_enabled:-missing}"
printf 'speed profile start: %sm at %sm/s\n' "${profile_start_distance:-missing}" "${profile_start_speed:-missing}"
printf 'speed profile cruise: %sm/s\n' "${profile_cruise_speed:-missing}"
printf 'speed profile approach: %sm to %sm/s\n' "${profile_approach_distance:-missing}" "${profile_approach_min:-missing}"
...
grep -q '3.14' <<<"${yaw_tolerance}" || failed=true
...
grep -q '0.02' <<<"${adapter_spin_effective_rate}" || failed=true
grep -q '1.5' <<<"${adapter_spin_timeout_factor}" || failed=true
grep -q '5.0' <<<"${adapter_spin_timeout_margin}" || failed=true
grep -q 'True' <<<"${adapter_use_map_spin}" || failed=true
grep -q '/cmd_vel_nav' <<<"${adapter_map_spin_topic}" || failed=true
grep -q '0.18' <<<"${adapter_map_spin_speed}" || failed=true
grep -q '3.0' <<<"${adapter_reverse_allowance}" || failed=true
grep -q 'False' <<<"${adapter_use_terminal_handoff}" || failed=true
grep -q '0.35' <<<"${adapter_terminal_handoff}" || failed=true
grep -q '0.05' <<<"${adapter_terminal_tolerance}" || failed=true
grep -q '0.05' <<<"${adapter_final_position_tolerance}" || failed=true
grep -q '3' <<<"${adapter_final_confirmations}" || failed=true
grep -q '3.0' <<<"${adapter_terminal_allowance}" || failed=true
grep -q '3.0' <<<"${adapter_collision_abort}" || failed=true
...
grep -q '0.75' <<<"${slowdown_ratio}" || failed=true
grep -q '10' <<<"${slowdown_max_points}" || failed=true
grep -q 'True' <<<"${reverse_straight_only}" || failed=true
grep -q '/cmd_vel_nav_smoothed' <<<"${pre_collision_topic}" || failed=true
...
grep -q 'True' <<<"${profile_enabled}" || failed=true
grep -q '0.3' <<<"${profile_start_distance}" || failed=true
grep -q '0.08' <<<"${profile_start_speed}" || failed=true
grep -q '0.4' <<<"${profile_cruise_speed}" || failed=true
grep -q '0.8' <<<"${profile_approach_distance}" || failed=true
grep -q '0.05' <<<"${profile_approach_min}" || failed=true
...
grep -q '0.08' <<<"${sonar_self_echo_min}" || failed=true
grep -q '0.12' <<<"${sonar_self_echo_max}" || failed=true
grep -q '0.15' <<<"${sonar_setback}" || failed=true
