...
def main():
    args = [
        ...
        "-p", "ultrasonic_turn_allow_distance:=0.25",
        "-p", "ultrasonic_self_echo_enabled:=true",
        "-p", "ultrasonic_self_echo_min_distance:=0.08",
        "-p", "ultrasonic_self_echo_max_distance:=0.12",
        "-p", "ultrasonic_sensor_to_rear_edge:=0.15",
    ...
    try:
        ...
        wait_until(
            ...
            and "self_echo_m=[0.080,0.120]" in driver.ultrasonic_status,
        ...
        # The installed rear sonar reports the chassis self-echo at about
        # 0.10 m.  It must be classified for operator visibility but ignored
        # for latch updates: a self-echo neither creates a false obstacle nor
        # clears a previously confirmed real obstacle.
        driver.ranges[1] = 0.10
        wait_until(
            lambda: driver.reverse_allowed is True
            and driver.left_turn_allowed is True
            and driver.right_turn_allowed is True
            ...
            2.0, "sensor_2 self-echo exclusion")
        driver.ranges[1] = math.inf
        wait_until(
            lambda: driver.reverse_allowed is True
            and driver.recent_nonzero(time.monotonic() - 0.30),
            2.0, "self-echo exclusion remains clear")
        ...
        wait_until(
            lambda: "rear ultrasonic blocked right sensor_2" in driver.status
            and driver.recent_all_zero(time.monotonic() - 0.20),
            2.0, "sensor_2 reverse stop")
        wait_until(
            lambda: "state=BLOCKED" in driver.ultrasonic_status
            and "blocking_sides=[RIGHT]" in driver.ultrasonic_status
        ...
        wait_until(
            lambda: "rear ultrasonic turn-distance left sensor_1"
            in driver.status
            and driver.left_turn_allowed is False
            and driver.right_turn_allowed is False
            and driver.recent_all_zero(time.monotonic() - 0.20),
            2.0, "left obstacle stops left turn")

        # Both turn directions now require both rear sectors, so the same
        # left obstacle must also stop a right turn.
        driver.command.angular.z = -0.08
        wait_until(
            lambda: driver.recent_all_zero(time.monotonic() - 0.20),
            2.0, "left obstacle also stops right turn")
        # At 0.26 m, both turns must recover while straight reverse remains
        # latched until the stricter 0.35 m reverse-clear threshold.
        driver.ranges[0] = 0.26
        wait_until(
            lambda: driver.left_turn_allowed is True
            and driver.right_turn_allowed is True
            and driver.reverse_allowed is False
            and "state=TURN_ONLY" in driver.ultrasonic_status
            and "turn_allow_m=0.250" in driver.ultrasonic_status
            and driver.recent_nonzero(time.monotonic() - 0.30),
            6.0, "turns recover above 0.25m while reverse stays blocked")

        print(
            "REAR_ULTRASONIC_INTEGRATION_PASS "
            "maneuver_permissions=PASS self_echo=PASS hard_stop=PASS "
            "reverse_taper=PASS latch=PASS "
            "no_echo_release=PASS operator_heartbeat=PASS "
            "dual_side_turn_guard=PASS side_reason=PASS "
            "turn_allow_0.25m=PASS reverse_clear_0.35m=PASS "
            "goal_lease=PASS "
