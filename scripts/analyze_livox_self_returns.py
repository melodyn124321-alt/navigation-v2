...
def main():
    ...
    parser.add_argument("--base-x", type=float, default=0.10)
    parser.add_argument("--base-y", type=float, default=-0.11)
    parser.add_argument("--base-z", type=float, default=0.749)
    parser.add_argument("--base-roll", type=float, default=-45.937)
    parser.add_argument("--base-pitch", type=float, default=-0.4065)
    # M12 (-X) faces vehicle left (+Y), so LiDAR +X faces vehicle right.
    ...
    for voxel, center in persistent_centers.items():
        ...
        if all(
            ...
            in zip(base_point, chassis_limits)
        ):
            chassis_voxels[voxel] = center
    print(
        "chassis_geometry: "
        "x[-0.38,+0.38] y[-0.27,+0.27] z[-0.05,+0.40]m"
    )
    if chassis_voxels:
        centers = list(chassis_voxels.values())
        ...
        print(
            f"chassis_self_returns: voxels={len(chassis_voxels)} "
            f"persistence_max="
            f"{max(persistent[voxel] for voxel in chassis_voxels)}"
        ...
        print(
            "chassis_self_returns: none in persistent voxels; "
            "no chassis mask is required"
        )
    ...
    if args.check_range_gate:
        ...
        if outside:
            ...
            print(
                f"RESULT: FAIL chassis self-returns have "
