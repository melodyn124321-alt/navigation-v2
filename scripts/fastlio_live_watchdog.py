...
class FastlioWatchdog(Node):
    def __init__(self):
        ...
        self.declare_parameter("session_gap_sec", 2.0)
        ...
        self.session_gap = self.get_parameter("session_gap_sec").value
        ...
        self.get_logger().info(
            ...
            f"grace={self.startup_grace:.1f}s "
            f"session_gap={self.session_gap:.1f}s "
    ...
    def reset_session(self, reason):
        ...
        self.started = time.monotonic()
        self.previous = None
        self.no_effective.clear()
        self.get_logger().info(
            f"new FAST-LIO session detected ({reason}); "
            f"odometry baseline cleared and {self.startup_grace:.1f}s "
            "startup grace restarted")
    ...
    def on_odometry(self, message):
        ...
        if (
            ...
            and receipt - self.previous_receipt > self.session_gap
        ):
            self.reset_session(
                f"odometry silence "
                f"{receipt - self.previous_receipt:.3f}s")
    ...
    def open_log_if_needed(self):
        ...
        replaced = self.log_handle is None or identity != self.log_identity
        truncated = (
            not replaced
            and self.log_last_size is not None
            and stat.st_size < self.log_last_size
        )
        if replaced or truncated:
            ...
            if truncated:
                self.reset_session("FAST-LIO log truncated")
            elif self.log_last_size is not None:
                self.reset_session("FAST-LIO log replaced")
        self.log_last_size = stat.st_size
    ...
    def poll_log(self):
        ...
        try:
            self.log_last_size = os.fstat(self.log_handle.fileno()).st_size
