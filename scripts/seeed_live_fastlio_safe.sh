...
VIZ_LOG="${ROS_ROOT}/logs/fastlio_viz_cloud_throttle.log"
...
VIZ_PID_FILE="${ROS_ROOT}/logs/fastlio_viz_cloud_throttle.pid"
VIZ_SCRIPT="${ROS_ROOT}/scripts/fastlio_viz_cloud_throttle.py"
...
case "${ACTION}" in
  start)
    ...
    [[ -x "${VIZ_SCRIPT}" ]] || {
      echo "Missing RViz cloud throttle: ${VIZ_SCRIPT}" >&2
      exit 1
    }
    ...
    : > "${WATCHDOG_LOG}"
    : > "${VIZ_LOG}"
    stale_viz_pid="$(read_pid "${VIZ_PID_FILE}")"
    stop_pid "${stale_viz_pid}"
    rm -f "${VIZ_PID_FILE}"
    ...
    nohup "${ROS_ENV}" python3 "${VIZ_SCRIPT}" \
      >>"${VIZ_LOG}" 2>&1 </dev/null &
    viz_pid=$!
    printf '%s\n' "${viz_pid}" > "${VIZ_PID_FILE}"
    ...
    for _ in $(seq 1 60); do
      if [[ -n "$(mapping_pids)" ]] \
        && pid_alive "${watchdog_pid}" \
        && pid_alive "${viz_pid}"; then
        echo "RUNNING launch_pid=${launch_pid} watchdog_pid=${watchdog_pid} viz_pid=${viz_pid} mapping_pids=$(mapping_pids | tr '\n' ',')"
        exit 0
      fi
  ...
  stop)
    ...
    rm -f "${WATCHDOG_PID_FILE}" "${LAUNCH_PID_FILE}" "${VIZ_PID_FILE}"
    echo "STOPPED FAST-LIO, watchdog, and RViz cloud throttle; Livox publisher and rosbag were not signalled."
    ;;
  status)
    watchdog_pid="$(read_pid "${WATCHDOG_PID_FILE}")"
    launch_pid="$(read_pid "${LAUNCH_PID_FILE}")"
    ...
    if pid_alive "${viz_pid}"; then
      echo "viz_throttle=RUNNING pid=${viz_pid}"
    else
      echo "viz_throttle=STOPPED"
    fi
    if [[ -s "${TRIP_FILE}" ]]; then
      echo "trip=YES"
      cat "${TRIP_FILE}"
