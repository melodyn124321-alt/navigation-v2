...
start_job() {
  ...
  # Clear stale CLI entities before graph-isolation checks. A previous
  # ros2 topic echo can otherwise appear as a second /goal_pose subscriber.
  "${ROS_ENV}" env FASTRTPS_DEFAULT_PROFILES_FILE="${NAV_FASTDDS_PROFILE}" \
    ROS2CLI_DISABLE_DAEMON=1 ros2 daemon stop >/dev/null 2>&1 || true
  sleep 3
...
wait_job() {
  ...
  emit_new_log() {
    ...
    total_lines="$(awk 'END {print NR + 0}' "${LOG_FILE}")"
    if (( total_lines + 1 < next_log_line )); then
      # A new `start` truncates the log. Restart streaming at its first line.
      next_log_line=1
    fi
    if (( total_lines >= next_log_line )); then
      sed -n "${next_log_line},${total_lines}p" "${LOG_FILE}"
      next_log_line=$((total_lines + 1))
    fi
  }

  printf 'WAIT_STREAM timeout=%ss log_file=%s\n' \
    "${timeout_sec}" "${LOG_FILE}"

  while (( SECONDS < deadline )); do
    # Always flush the activation log before checking EXIT_FILE. This preserves
    # the full stage/error history even when `wait` starts after the worker has
    ...
    if (( SECONDS >= next_report )); then
      stage="$(grep -E '^\[[0-9]+/11\]' "${LOG_FILE}" 2>/dev/null \
        | tail -n 1 || true)"
      printf '[WAIT_HEARTBEAT] elapsed=%ss remaining=%ss stage=%s\n' \
        "$((SECONDS - wait_started))" "$((deadline - SECONDS))" \
        "${stage:-starting}"
      atomic_write "${STATUS_FILE}" \
        "RUNNING pid=${pid:-unknown} elapsed=$((SECONDS - wait_started))s stage=${stage:-starting}"
