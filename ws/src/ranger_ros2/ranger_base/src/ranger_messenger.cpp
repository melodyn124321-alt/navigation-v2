...
void RangerROSMessenger::PublishStateToROS() {
  ...
  // A single control-mode request at process startup can be lost while the
  // Ranger controller is still completing its CAN/protocol handshake.  Never
  // assume that error_code == 0 also means CAN command mode: RC mode reports
  // no error but ignores ROS /cmd_vel.  Retry immediately and then about once
  // per second until the controller confirms CONTROL_MODE_CAN.
  static uint32_t command_mode_retry_count = 0;
  if (state.system_state.control_mode != SystemState::CONTROL_MODE_CAN) {
    if (command_mode_retry_count % std::max(1, update_rate_) == 0) {
      RCLCPP_WARN(
          node_->get_logger(),
          "Ranger control_mode=%u is not CAN; requesting CAN command mode",
          static_cast<unsigned int>(state.system_state.control_mode));
      robot_->EnableCommandedMode();
    }
    ++command_mode_retry_count;
  } else {
    command_mode_retry_count = 0;
  }
