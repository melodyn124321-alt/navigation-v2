...
std::vector<float> symmetricOffsets(double range, double step)
{
  std::vector<float> offsets{0.0F};
  if (range <= 0.0 || step <= 0.0) {
    return offsets;
  }
  for (double value = step; value <= range + 1.0e-9; value += step) {
    offsets.push_back(static_cast<float>(value));
    offsets.push_back(static_cast<float>(-value));
  }
  return offsets;
}
...
public:
  PcdNdtLocalizer() : Node("pcd_ndt_localizer")
  {
    ...
    scan_accumulation_frames_ = std::max(
      1, static_cast<int>(declare_parameter<std::int64_t>(
        "scan_accumulation_frames", 5)));
    ...
    robust_fitness_enable_ = declare_parameter<bool>("robust_fitness_enable", true);
    robust_trim_fraction_ = declare_parameter<double>("robust_trim_fraction", 0.72);
    robust_inlier_distance_m_ =
      declare_parameter<double>("robust_inlier_distance_m", 0.50);
    robust_min_inlier_ratio_ =
      declare_parameter<double>("robust_min_inlier_ratio", 0.55);
    robust_max_rmse_ = declare_parameter<double>("robust_max_rmse", 0.18);
    initial_search_enable_ = declare_parameter<bool>("initial_search_enable", true);
    initial_search_xy_range_m_ =
      declare_parameter<double>("initial_search_xy_range_m", 1.0);
    initial_search_xy_step_m_ =
      declare_parameter<double>("initial_search_xy_step_m", 1.0);
    initial_search_yaw_range_rad_ =
      declare_parameter<double>("initial_search_yaw_range_rad", 0.261799388);
    initial_search_yaw_step_rad_ =
      declare_parameter<double>("initial_search_yaw_step_rad", 0.130899694);
    initial_search_leaf_size_ =
      declare_parameter<double>("initial_search_leaf_size", 0.50);
    initial_search_max_iterations_ =
      declare_parameter<int>("initial_search_max_iterations", 15);
    initial_search_max_candidates_ =
      declare_parameter<int>("initial_search_max_candidates", 45);
    initial_search_max_fitness_score_ =
      declare_parameter<double>("initial_search_max_fitness_score", 0.10);
    initial_confirm_frames_ = declare_parameter<int>("initial_confirm_frames", 3);
    initial_confirm_translation_m_ =
      declare_parameter<double>("initial_confirm_translation_m", 0.35);
    initial_confirm_rotation_rad_ =
      declare_parameter<double>("initial_confirm_rotation_rad", 0.174532925);
    ...
    RCLCPP_INFO(
      get_logger(),
      "PCD NDT localization ready. map=%s points=%zu scan_accumulation_frames=%d "
      "require_initial_pose=%d reject_occupied_pose=%d",
      map_path_.c_str(), map_cloud_->size(), scan_accumulation_frames_,
      require_initial_pose_, reject_occupied_pose_);
  }
...
private:
  ...
  void initialPoseCallback(const geometry_msgs::msg::PoseWithCovarianceStamped::SharedPtr msg)
  {
    ...
    const Eigen::Matrix4f initial_pose = poseWithCovToMatrix(*msg) * base_t_localization;
    {
      ...
      initial_search_pending_ = initial_search_enable_;
      initial_search_quality_passed_ = !initial_search_enable_;
      initial_confirm_count_ = 0;
      last_fitness_ = 999.0;
  ...
  Eigen::Matrix4f runBoundedInitialSearch(
    const CloudT::ConstPtr & scan,
    const Eigen::Matrix4f & center_guess)
  {
    if (!initial_search_enable_ || !initial_search_pending_) {
      return center_guess;
    }
    initial_search_pending_ = false;

    const auto x_offsets =
      symmetricOffsets(initial_search_xy_range_m_, initial_search_xy_step_m_);
    const auto y_offsets =
      symmetricOffsets(initial_search_xy_range_m_, initial_search_xy_step_m_);
    const auto yaw_offsets =
      symmetricOffsets(initial_search_yaw_range_rad_, initial_search_yaw_step_rad_);
    const int candidate_limit = std::max(1, initial_search_max_candidates_);

    CloudT::Ptr coarse_scan = voxelDownsample(scan, initial_search_leaf_size_);
    pcl::NormalDistributionsTransform<PointT, PointT> search_ndt;
    search_ndt.setTransformationEpsilon(std::max(0.02, ndt_trans_eps_));
    search_ndt.setStepSize(std::max(0.1, ndt_step_size_));
    search_ndt.setResolution(ndt_resolution_);
    search_ndt.setMaximumIterations(std::max(1, initial_search_max_iterations_));
    search_ndt.setInputTarget(map_cloud_);
    search_ndt.setInputSource(coarse_scan);

    const Eigen::Matrix4f localization_to_base = localizationToBase();
    const Eigen::Matrix4f base_to_localization = localization_to_base.inverse();
    const Eigen::Matrix4f center_base = center_guess * localization_to_base;
    Eigen::Matrix4f best_guess = center_guess;
    double best_score = std::numeric_limits<double>::infinity();
    int evaluated = 0;
    int converged = 0;

    for (const float yaw_offset : yaw_offsets) {
      for (const float x_offset : x_offsets) {
        for (const float y_offset : y_offsets) {
          if (evaluated >= candidate_limit) {
            break;
          }
          Eigen::Matrix4f candidate_base = center_base;
          candidate_base.block<3, 3>(0, 0) =
            Eigen::AngleAxisf(yaw_offset, Eigen::Vector3f::UnitZ()).toRotationMatrix() *
            center_base.block<3, 3>(0, 0);
          candidate_base(0, 3) += x_offset;
          candidate_base(1, 3) += y_offset;
          const Eigen::Matrix4f candidate = candidate_base * base_to_localization;

          CloudT aligned;
          search_ndt.align(aligned, candidate);
          ++evaluated;
          if (!search_ndt.hasConverged()) {
            continue;
          }
          ++converged;
          const double score = search_ndt.getFitnessScore();
          if (std::isfinite(score) && score < best_score) {
            best_score = score;
            best_guess = search_ndt.getFinalTransformation();
          }
        }
        if (evaluated >= candidate_limit) {
          break;
    ...
    RCLCPP_WARN(
      get_logger(),
      "Initial pose bounded search: evaluated=%d converged=%d best_fitness=%.4f "
      "gate=%.4f quality=%s best_body=(%.2f,%.2f) window_xy=+/-%.2fm "
      "window_yaw=+/-%.1fdeg",
      evaluated, converged, best_score, initial_search_max_fitness_score_,
      best_score <= initial_search_max_fitness_score_ ? "PASS" : "FAIL",
      best_guess(0, 3), best_guess(1, 3),
      initial_search_xy_range_m_,
      initial_search_yaw_range_rad_ * 180.0 / M_PI);
    initial_search_quality_passed_ =
      std::isfinite(best_score) && best_score <= initial_search_max_fitness_score_;
    return best_guess;
  }
  ...
  RobustFitness evaluateRobustFitness(const CloudT & aligned) const
  {
    RobustFitness result;
    if (!robust_fitness_enable_ || aligned.empty() || !map_cloud_ || map_cloud_->empty()) {
      return result;
    }

    std::vector<double> distances;
    distances.reserve(aligned.size());
    ...
    for (const auto & point : aligned.points) {
      ...
      if (map_tree_.nearestKSearch(point, 1, index, squared_distance) != 1) {
        continue;
      }
      const double distance = std::sqrt(std::max(0.0F, squared_distance.front()));
      distances.push_back(distance);
      if (distance <= robust_inlier_distance_m_) {
        ++inlier_count;
      }
    }
    if (distances.size() < static_cast<std::size_t>(min_scan_points_)) {
      return result;
    }

    std::sort(distances.begin(), distances.end());
    const double trim_fraction = std::clamp(robust_trim_fraction_, 0.10, 1.0);
    const std::size_t trim_count = std::max<std::size_t>(
      static_cast<std::size_t>(min_scan_points_),
      static_cast<std::size_t>(std::floor(distances.size() * trim_fraction)));
    double sum_squared = 0.0;
    for (std::size_t i = 0; i < std::min(trim_count, distances.size()); ++i) {
      sum_squared += distances[i] * distances[i];
    }
    result.trimmed_rmse = std::sqrt(
      sum_squared / static_cast<double>(std::min(trim_count, distances.size())));
    result.inlier_ratio =
      static_cast<double>(inlier_count) / static_cast<double>(distances.size());
    result.valid = std::isfinite(result.trimmed_rmse);
    return result;
  }

  CloudT::Ptr accumulateScanInCurrentBody(const CloudT::Ptr & current_scan)
  {
    Eigen::Matrix4f current_scan_odom = Eigen::Matrix4f::Identity();
    bool have_scan_odom = false;
    std::deque<ScanHistoryEntry> history;
    {
      std::lock_guard<std::mutex> lock(mutex_);
      current_scan_odom = current_odom_;
      have_scan_odom = have_odom_;

      if (!scan_history_.empty() && have_scan_odom) {
        const Eigen::Matrix4f odom_delta =
          scan_history_.back().odom.inverse() * current_scan_odom;
        const double translation = odom_delta.block<3, 1>(0, 3).norm();
        const Eigen::AngleAxisf rotation(odom_delta.block<3, 3>(0, 0));
        if (translation > 0.50 || std::abs(rotation.angle()) > 0.52) {
          RCLCPP_WARN(
            get_logger(),
            "FAST-LIO odometry discontinuity while accumulating scans: "
            "translation=%.3fm rotation=%.1fdeg; clearing scan history",
            translation, std::abs(rotation.angle()) * 180.0 / M_PI);
          scan_history_.clear();
        }
      }

      scan_history_.push_back({current_scan, current_scan_odom, have_scan_odom});
      while (scan_history_.size() > static_cast<std::size_t>(scan_accumulation_frames_)) {
        scan_history_.pop_front();
      }
      history = scan_history_;
    }

    CloudT::Ptr accumulated(new CloudT);
    for (const auto & entry : history) {
      if (have_scan_odom && entry.have_odom) {
        CloudT compensated;
        const Eigen::Matrix4f entry_to_current =
          current_scan_odom.inverse() * entry.odom;
        pcl::transformPointCloud(*entry.cloud, compensated, entry_to_current);
        *accumulated += compensated;
      } else if (&entry == &history.back()) {
        *accumulated += *entry.cloud;
      }
    }
    return accumulated;
  }
  ...
  void cloudCallback(const sensor_msgs::msg::PointCloud2::SharedPtr msg)
  {
    ...
    CloudT::Ptr accumulated = accumulateScanInCurrentBody(scan_raw);
    CloudT::Ptr scan = voxelDownsample(accumulated, scan_leaf_size_);
    ...
    Eigen::Matrix4f guess = makeInitialGuess();
    guess = runBoundedInitialSearch(scan, guess);
    CloudT aligned;
    ndt_.setInputSource(scan);
    ndt_.align(aligned, guess);

    const double raw_fitness = ndt_.getFitnessScore();
    const RobustFitness robust = evaluateRobustFitness(aligned);
    const bool robust_quality_ok =
      robust.valid && robust.inlier_ratio >= robust_min_inlier_ratio_ &&
      robust.trimmed_rmse <= robust_max_rmse_;
    const double fitness = robust_quality_ok ? robust.trimmed_rmse : raw_fitness;
    // A coarse bounded search can fail its quality gate even when the
    // subsequent full-resolution NDT result is excellent.  The old logic
    // then kept acquisition_quality_ok=false forever, so every later scan was
    // rejected regardless of a 0.02-style fitness score.  Let a converged
    // full-resolution result that passes the stricter acquisition threshold
    // recover the acquisition state.
    const bool full_resolution_acquisition_ok =
      ndt_.hasConverged() &&
      ((std::isfinite(raw_fitness) && raw_fitness <= initial_search_max_fitness_score_) ||
      robust_quality_ok);
    if (initial_search_enable_ && !initial_search_quality_passed_ &&
        full_resolution_acquisition_ok)
    {
      initial_search_quality_passed_ = true;
      RCLCPP_WARN(
        get_logger(),
        "Initial acquisition recovered from full-resolution NDT: "
        "raw_fitness=%.4f robust_rmse=%.4f inliers=%.1f%%",
        raw_fitness, robust.trimmed_rmse, robust.inlier_ratio * 100.0);
    }
    const bool acquisition_quality_ok =
      have_ndt_pose_ || !initial_search_enable_ ||
      initial_search_quality_passed_ || full_resolution_acquisition_ok;
    const bool ok =
      ndt_.hasConverged() &&
      ((std::isfinite(raw_fitness) && raw_fitness <= max_fitness_score_) ||
      robust_quality_ok) && acquisition_quality_ok;
    if (!ok) {
      RCLCPP_WARN_THROTTLE(get_logger(), *get_clock(), 1000,
        "NDT rejected: converged=%d raw_fitness=%.4f robust_rmse=%.4f "
        "inliers=%.1f%% requirements=%.4f/%.1f%% points=%zu",
        ndt_.hasConverged(), raw_fitness, robust.trimmed_rmse,
        robust.inlier_ratio * 100.0, robust_max_rmse_,
        robust_min_inlier_ratio_ * 100.0, scan->size());
    ...
    if (!have_ndt_pose_ && initial_confirm_frames_ > 1) {
      Eigen::Matrix4f held_pose;
      int confirmation_count = 0;
      {
        std::lock_guard<std::mutex> lock(mutex_);
        bool stable = initial_confirm_count_ == 0;
        if (initial_confirm_count_ > 0) {
          const Eigen::Matrix4f delta = pending_initial_pose_.inverse() * pose;
          const double translation = delta.block<3, 1>(0, 3).norm();
          const Eigen::AngleAxisf angle_axis(delta.block<3, 3>(0, 0));
          stable =
            translation <= initial_confirm_translation_m_ &&
            std::abs(angle_axis.angle()) <= initial_confirm_rotation_rad_;
        }
        if (!stable) {
          initial_confirm_count_ = 0;
        }
        pending_initial_pose_ = pose;
        ++initial_confirm_count_;
        confirmation_count = initial_confirm_count_;
        held_pose = map_t_body_;
        // Keep using the latest good candidate as the prediction origin while
        // acquisition is being confirmed. Previously every confirmation frame
        // restarted from the raw RViz click, so a valid first match was lost
        // immediately on the next non-repetitive Livox scan.
        map_t_body_ = pose;
        if (have_odom_) {
          last_odom_ = current_odom_;
          have_last_odom_ = true;
        ...
        last_fitness_ = std::max(fitness, max_fitness_score_ + 1.0e-6);
        last_status_ =
          "NDT VERIFYING " + std::to_string(confirmation_count) + "/" +
          std::to_string(initial_confirm_frames_);
      }
      if (confirmation_count < initial_confirm_frames_) {
        publishPoseEstimate(
          msg->header.stamp, held_pose,
          std::max(fitness, max_fitness_score_ + 1.0e-6),
          "NDT VERIFYING " + std::to_string(confirmation_count) + "/" +
          std::to_string(initial_confirm_frames_),
          false);
        return;
  ...
  bool robust_fitness_enable_;
  double robust_trim_fraction_;
  double robust_inlier_distance_m_;
  double robust_min_inlier_ratio_;
  double robust_max_rmse_;
  bool initial_search_enable_;
  double initial_search_xy_range_m_;
  double initial_search_xy_step_m_;
  double initial_search_yaw_range_rad_;
  double initial_search_yaw_step_rad_;
  double initial_search_leaf_size_;
  int initial_search_max_iterations_;
  int initial_search_max_candidates_;
  double initial_search_max_fitness_score_;
  int initial_confirm_frames_;
  double initial_confirm_translation_m_;
  double initial_confirm_rotation_rad_;
  ...
  bool initial_search_pending_ = false;
  bool initial_search_quality_passed_ = false;
  int initial_confirm_count_ = 0;
  Eigen::Matrix4f pending_initial_pose_ = Eigen::Matrix4f::Identity();
  std::deque<ScanHistoryEntry> scan_history_;
